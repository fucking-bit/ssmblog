            ...
            <form class="layui-form">
                <fieldset class="layui-elem-field" style="border-color: #4E5465">
                    ...
                    <div class="layui-field-box">
                        <input type="title" name="title" required lay-verify="title" placeholder="请输入标题" autocomplete="off" class="layui-input">
                        <textarea id="content" name="content" required lay-verify="content"></textarea>
                    </div>
                </fieldset>
                <div class="layui-input-block" style="position:absolute;right: 20px">
                    <button class="layui-btn"  lay-submit lay-filter="submit">立即发布</button>
                </div>
            </form>
        </div>
...
<script>
    ...
    layui.use(['form','layedit'], function(){
        ...
        form.on('submit(submit)', function(data){
            var data1 = {title:data.field.title,content:layedit.getContent(index)}
            console.log(data1)
            $.ajax({
                type:"post",
