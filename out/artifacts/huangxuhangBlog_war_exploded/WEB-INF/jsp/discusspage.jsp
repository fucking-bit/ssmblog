                    ...
                    <ul>
                        <c:forEach items="${discuss}" var="item">
                            <li style="margin: 4px 0px;border-bottom: 1px dotted #cccccc;padding: 4px;">
                                <a href="${pageContext.request.contextPath}/discuss/ScanDiscuss?id=<c:out value="${item.id}" escapeXml="false"></c:out>" ><span><c:out value="${item.content}" escapeXml="false"></c:out></span></a>
                                <span style="position: absolute;right: 10px;">[<c:out value="${item.createTime}" escapeXml="false"></c:out>]</span>
                            </li>
                        </c:forEach>
                    </ul>
                </div>
...
<script>
    ...
    layui.use(['form'], function(){
        ...
        form.on('submit(submit)', function(data){
            var data1 = {content:data.field.content}
            console.log(data1) //当前容器的全部表单字段，名值对形式：{name: value}
            $.ajax({
                type:"post",
                url:"${pageContext.request.contextPath}/discuss/PublishTalk",
                data:data1,
